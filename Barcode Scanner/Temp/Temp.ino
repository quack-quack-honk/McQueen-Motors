...
// This program takes an input from a sensor which returns either a HIGH or a LOW value
...
// If the bits in the array are incorrect then the program will output an error and flip the original array
// The program will perform the decoding algorithm on the new flipped array to check if the barcode is the opposite way around
// The program will output the decimal number
// If the array is still incorrect then there is an error message
